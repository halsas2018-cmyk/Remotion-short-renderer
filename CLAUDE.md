...
- **Horizon 3 — 🟡 PARTIALLY DONE.** 3.1/3.2/3.3 ✅, 3.4 data-only (`pacing` emitted but orchestrator ignores), 3.5 dropped, 3.6 ✅ (audio-text alignment: `beat_generator.py::align_text_to_audio_window` re-derives each beat's `text` from the audio words in its frame window; original chunk preserved as `scriptText` for the LLM metadata path), 3.8 ✅ (beat-type decision rules: new `BEAT_TYPE_DECISION_RULES` constant in `beat_generator.py` injected into the MODE A prompt between EXAMPLES and arc/diversity; 3 families — comparison, data-vis, text-on-card — with per-type "use when" rules; addresses the 4 boundary cases where `BEAT_TYPE_FIELD_HINTS` and `BEAT_TYPE_EXAMPLES` alone don't disambiguate; prompt-only, ~500 tokens, free-tier safe; empty constant collapses to zero-cost via `if BEAT_TYPE_DECISION_RULES:` guard). Next: 4 or finish 3.4 orchestrator wiring.
