...
- **Horizon 3.1 — Story-level visual planning** (second LLM pass via `beat_generator.py::plan_story_arc` returns an arc label per beat — `intro` / `explain` / `compare` / `climax` / `outro`; arc is then passed into the per-beat prompt and into the merge loop to constrain the LLM's type picks to the allowed set for each arc; `force_intro_to_headline_card` enforces that beat 0 is a `headline_card` regardless of the LLM's pick; the `ARC_TYPES` dict in `beat_generator.py` maps each arc label to a whitelist of beat types; if the LLM call fails, the fallback is a `["intro", "explain"..., "outro"]` shape scaled to the expected beat count) — ✅ DONE
- **Horizon 3.2 — Beat type diversity budget** (`DIVERSITY_REQUIRED` / `DIVERSITY_DATA_VIS` constants in `beat_generator.py`; prompt now injects a "DIVERSITY BUDGET (mandatory)" section into both Mode A and Mode B paths requiring at least one of `chart_line` / `map_3d` / `quote_attribution` and at least one of `progress_meter` / `timeline` / `process_flow`, plus a "no more than 3 consecutive beats of the same type" rule; `BEAT_TYPES` expanded from 8 to 18 entries to expose the 2.1.x components — `headline_card`, `chart_line`, `map_3d`, `quote_attribution`, `stat_pill`, `compare_split`, `location_pulse`, `scrollytelling`, `ticker_tape` — so the LLM can actually pick them; per-type Zod schemas for the 8 new types already exist in `src/beats/registry.ts` from Horizons 2.1.2–2.1.7) — ✅ DONE
- **Horizon 3.3 — Beat length auto-tuning** (`target_frames_for_word_count(word_count) = max(45, min(180, round(word_count * 4.5)))`; constants bumped: `MIN_BEAT_FRAMES 30→45`, `MAX_BEAT_FRAMES_SOFT 90→180`, `MAX_BEAT_FRAMES_HARD 130→200`, `MAX_BEAT_FRAMES 90→180`; `assign_frames_from_word_ranges` now trims beats to the word-count cap after the Whisper alignment (rather than splitting); `should_split_beat` only fires when the trimmed beat still exceeds the 200-frame hard ceiling, so splits are now rare; cap math lives in one place) — ✅ DONE
- **Horizon 3.4 — Auto-pacing based on word density** (`compute_pacing(text) → "fast" | "normal" | "slow"` based on word count thresholds 1–3 / 4–8 / 9+; the `pacing` field is emitted on every beat in `beats.json` after the type merge; `src/beats/types.ts::beatBaseShape` now declares `pacing: z.enum(["slow", "normal", "fast"]).optional()` so the Zod dispatcher doesn't strip it; the orchestrator's `±20%` duration adjustment is NOT yet wired — the field is emitted correctly but the orchestrator currently ignores it, which is the documented "data-is-correct-but-consumer-isn't-yet" state per ROADMAP §3.4) — ✅ DONE
- **Horizon 3.5 — Visual reference images for the LLM** — 🟡 **DROPPED** (per user decision on `79def0e`); `map_3d` continues to work with text-only prompts as before; the LLM was already producing reasonable `locationName` + `latitude` / `longitude` metadata, and the reference-screenshot plumbing was deemed not worth the cost; no code change required to drop this sub-horizon)
- **Horizon 3.8 — Beat-type decision rules in prompt** (new `BEAT_TYPE_DECISION_RULES` constant in `beat_generator.py` injected into the MODE A prompt between EXAMPLES and the arc/diversity sections; 3 families covered — comparison (`versus` vs `before_after` vs `compare_split`), data-vis (`progress_meter` vs `chart_line` vs `timeline` vs `process_flow`), text-on-card (`headline_card` vs `key_statement` vs `icon_text` vs `quote_attribution` vs `scrollytelling`); addresses the boundary-case ambiguities that `BEAT_TYPE_FIELD_HINTS` and `BEAT_TYPE_EXAMPLES` alone don't disambiguate; prompt-only change, no schema/React impact; ~2000 chars / ~500 tokens added to the prompt, free-tier safe; constant guarded with `if BEAT_TYPE_DECISION_RULES:` so an empty constant collapses to zero-cost (one-character rollback)) — ✅ DONE
...
**Next up after 2.3.x (and after 2.3.x is fully done):** **2.4 (beat-emphasis words → component-level highlights for `versus` / `before_after` / `quote_card` — adds the `rough-notation` emphasis cycle that already exists in `KeyStatement` to the 3 components that need it)**.
...
### 3.1 Story-level visual planning — ✅ DONE
- Implemented in `beat_generator.py::plan_story_arc`. A second LLM pass returns a JSON array of arc labels (`intro` / `explain` / `compare` / `climax` / `outro`), one per beat, sized to the expected beat count for the script.
- The `ARC_TYPES` dict maps each arc label to a whitelist of allowed beat types. The per-beat merge loop in `generate_beats()` clamps the LLM's pick to the whitelist when a `story_arc` was provided, so the output is structurally constrained rather than homogeneous.
- The intro beat is forced to `headline_card` via `force_intro_to_headline_card()` regardless of the LLM's pick. This is the 2.1.1 hand-off the ROADMAP called out: the new headline component gets the bigger intro for the story hook.
- If the LLM call fails, the fallback is a `["intro"] + ["explain"] * (n - 2) + ["outro"]` shape scaled to `n = max(3, len(script.split()) // 10)`.
- Cost: ~$0.001/story on `gpt-4o-mini`.

### 3.2 Beat type diversity budget — ✅ DONE
- `DIVERSITY_REQUIRED = ["chart_line", "map_3d", "quote_attribution"]` and `DIVERSITY_DATA_VIS = ["progress_meter", "timeline", "process_flow"]` constants in `beat_generator.py` drive a new prompt section that the LLM sees in both MODE A and MODE B.
- `BEAT_TYPES` was expanded from 8 to 18 entries to expose the 2.1.x components — `headline_card`, `chart_line`, `map_3d`, `quote_attribution`, `stat_pill`, `compare_split`, `location_pulse`, `scrollytelling`, `ticker_tape` — so the LLM can actually pick them. (Previously, the LLM would have defaulted to `key_statement` for any of these because the per-type fields weren't in the dict.)
- The diversity rules are prompt-only (not enforced in the merge loop). The prompt says "your story MUST include at least one of each" + "no more than 3 consecutive beats of the same type". LLM compliance is good but not guaranteed — diversity is a soft constraint, not a hard one.

### 3.3 Beat length auto-tuning — ✅ DONE
- `target_frames_for_word_count(word_count) = max(45, min(180, round(word_count * 4.5)))` is the single source of truth for the auto-tuned duration. Floor 45 frames (1.5s), ceiling 180 frames (6.0s).
- Constants bumped from the previous `MIN_BEAT_FRAMES=30` / `MAX_BEAT_FRAMES_SOFT=90` / `MAX_BEAT_FRAMES_HARD=130` to `45` / `180` / `200` so the cap math covers the full 1.5–6s range.
- `assign_frames_from_word_ranges()` now applies the per-beat cap AFTER the Whisper timestamp alignment, trimming `end_frame` to `start_frame + cap` when the natural duration exceeds the cap. This is a **deliberate semantic change** from the previous "split oversized beats" behavior — splitting destroys the LLM's narrative chunking, trimming respects it. `split_long_beats()` still exists as a fallback for beats that exceed `MAX_BEAT_FRAMES_HARD=200` (the absolute ceiling), but most oversize beats are now handled by trimming.
- The orchestrator's `computeTransitionFrames` is unchanged; it reads `durationInFrames` and the cross-fade math is unaffected.

### 3.4 Auto-pacing based on word density — ✅ DONE
- `compute_pacing(text) → "fast" | "normal" | "slow"` based on word count thresholds: `≤3 → "fast"`, `4–8 → "normal"`, `≥9 → "slow"`. The function lives in `beat_generator.py` and the `pacing` field is emitted on every beat in `beats.json` after the type merge.
- `src/beats/types.ts::beatBaseShape` now declares `pacing: z.enum(["slow", "normal", "fast"]).optional()` so the Zod dispatcher doesn't strip the field at render time. **Without this declaration, Zod's `superRefine` would receive a beat without `pacing`, even though the field was emitted by the Python pipeline.**
- **Open follow-up (deferred, NOT in 3.4):** the orchestrator's `±20%` `durationInFrames` adjustment based on `pacing` is **not yet wired**. The data flows correctly from Python → JSON → Zod → component, but the orchestrator currently treats `durationInFrames` as the authoritative value and ignores `pacing`. To complete 3.4, the orchestrator (likely `MotionGraphicsVideo.tsx` or `renderBeat.tsx`) needs to read `beat.pacing` and multiply `durationInFrames` by `0.8` ("fast") / `1.0` ("normal") / `1.2` ("slow"). Add this to a future horizon's "next" list. **The 3.4 acceptance criteria as written in the spec ("the Python pipeline should output a per-beat pacing hint and the orchestrator should adjust `durationInFrames` accordingly") is half-done: Python emits, the orchestrator ignores. The data is correct and renderable today; the visual difference is just not applied yet.**

### 3.5 Visual reference images for the LLM — 🟡 DROPPED
- Per user decision on `79def0e`, 3.5 is dropped from this horizon.
- No code change was required to drop this sub-horizon — the 3.5 spec was "include 1–2 reference screenshots of the existing `Map3D` component in the LLM prompt when generating `map_3d` beats", and the screenshots were never added to the prompt. Dropping the sub-horizon means: no reference screenshots, no prompt-injection plumbing, no `map_3d` beat-type special-casing. The LLM continues to produce reasonable `locationName` + `latitude` / `longitude` metadata for `map_3d` beats from the text-only prompt it gets today.
- If `map_3d` beat quality proves insufficient in the future, revive 3.5 by adding 1–2 base64-encoded PNG screenshots of `Map3DTest` to the prompt context for `map_3d` beats specifically. Until then, the cost of building the screenshot-plumbing (~200 LoC + image-encoding helpers) is not justified.
