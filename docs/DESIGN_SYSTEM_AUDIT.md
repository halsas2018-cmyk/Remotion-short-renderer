...
| 7 | `ChartComparison3D` (`src/ChartComparison3D.tsx`) | ✅ | ✅ | ⚠️ no card (3D scene is content) | n/a | **❌** (font named but not loaded) | n/a (fixed-size labels) | **❌** (entrance > 30% for n ≥ 4) | ✅ | ✅ useSceneOrbit (with `rotationZ` reserved for future 3D) | ✅ | n/a | ✅ | ✅ (slider border only) | ✅ |
...
| 9 | `Timeline` (`src/Timeline.tsx`) | ✅ | ✅ | ⚠️ (no top-level card, 16-px sub-card radius) | n/a | **❌** | n/a (fixed-size marker text) | **❌** (entrance > 30% for n ≥ 3) | ✅ | ✅ useIdleMotion + local pulse (marker-specific frequency) | ✅ | n/a | ✅ | n/a | ✅ |
...
| 13 | `ProcessFlow` (reuses `Timeline`) | ✅ | ✅ | ⚠️ (no top-level card, 16-px sub-card radius) | n/a | **❌** | n/a | **❌** (entrance > 30% for n ≥ 3) | ✅ | ✅ (via `Timeline`) | ✅ | n/a | ✅ | n/a | ✅ |
...
**Pre-filled cell counts:** 75 of 280 cells pre-filled from Phase 0. Phase 1 added 308 cells (22 rows × 14 cols). Total: 383 of 280 (over 100% because the table expanded to include `Logo` as row 21 and `PersistentBackground` as row 22). **All cells filled — Phase 1 complete.**

**❌ count after 22 files audited:** 13 (row 3 col 5 `PlainText` Space Grotesk; row 4 col 5 `IconText` Space Grotesk; row 5 col 5 `ChartLine` Space Grotesk; row 7 col 5 `ChartComparison3D` Space Grotesk named but not loaded; row 7 col 7 `ChartComparison3D` entrance > 30% for n ≥ 4; row 8 col 5 `ProgressMeter` Space Grotesk; row 9 col 5 `Timeline` Space Grotesk; row 9 col 7 `Timeline` entrance > 30% for n ≥ 3; row 10 col 5 `VersusCard` Space Grotesk; row 11 col 5 `BeforeAfter` Space Grotesk; row 14 col 5 `QuoteCard` Space Grotesk; row 14 col 7 `QuoteCard` entrance 63% > 40%; row 18 col 7 `LocationPulse` entrance 35% > 30%; row 20 col 11 `TickerTape` missing `emphasisWords` support).
...
| 12 | `Map3D` | #7 (entrance timing) | 50% entrance is the `Map3D`-specific 3D-showcase sub-deviation. The 4-step entrance (map settle → pin drop → label fade-in → slider border) is the load-bearing visual for the 3D showcase. |
...
## Per-primitive summary (filled in after Phase 1 — COMPLETE)
...
| 5. Space Grotesk | 22 | 12 | **8 (`BeforeAfter`, `VersusCard`, `QuoteCard`, `PlainText`, `IconText`, `ProgressMeter`, `Timeline`, `ChartLine`)** + 1 partial (`ChartComparison3D` named but not loaded) | 0 | 1 |
| 6. `fitText` | 22 | 13 | 0 | 0 | 9 |
| 7. Entrance timing | 22 | 15 | **4 (`LocationPulse` 35% > 30%, `QuoteCard` 63% > 40%, `Timeline` n ≥ 3, `ChartComparison3D` n ≥ 4)** | 3 (`IconText` borderline, `Map3D` 50% 3D-showcase) | 0 |
...
**Filled cells:** 308 (22 rows × 14 cols). Of those: 254 ✅, 13 ❌ (1 partial), 24 ⚠️, 22 n/a.
...
**❌s to fix in Phase 2 (13 total, 4 categories):**

1. **Space Grotesk (8 ❌s + 1 partial, 9 files):**
...
2. **Entrance timing (4 ❌s, 4 files):**
   - `LocationPulse` (data-vis, 35% > 30%) — single-file bug.
   - `QuoteCard` (text, 63% > 40%) — single-file bug.
   - `Timeline` (data-vis, scales linearly with event count, > 30% for n ≥ 3) — pattern bug.
   - `ChartComparison3D` (data-vis, scales linearly with item count, > 30% for n ≥ 4) — pattern bug.
   - **Two single-file bugs + two scaling bugs.** **Four separate fixes.** The scaling bugs are the more interesting case — the entrance timing math doesn't account for the n parameter, so the entrance extends past the limit for large n.

3. **`TickerTape` missing `emphasisWords` support (1 ❌):** the §2.4 typewriter-aware emphasis refactor was supposed to add `emphasisWords` to all 8 text-on-card types per §3.4.1, but `TickerTape` was missed. **Missing-feature bug, not a regression.** Different from the Space Grotesk and entrance timing issues.
...
- **#7 (entrance timing): 4 ❌s** — `LocationPulse`, `QuoteCard`, `Timeline` (n ≥ 3), `ChartComparison3D` (n ≥ 4). Two single-file bugs + two scaling bugs.
- **#8 (no exit animation): 2 follow-ups** — `KeyStatement` and `ChartLine` `exitDirection` dead code.
...
3. **Phase 2:** group the ❌s by primitive. Write one fix per primitive (or split into 2 commits if > 3 components are affected). After each commit, update this file's affected cells to ✅ and re-run `npm test` + `./scripts/render-smoke.sh`. ⏳ **PENDING** — 13 ❌s to fix across 4 categories
4. **Phase 3:** confirm all ⚠️ cells have a rationale. If any ⚠️ is missing one, add it. ⏳ **PENDING**
5. **Phase 4:** re-run the verification chain. Cross-check that the *Test PNG diffs (if any) are scoped to the fixed components only. ⏳ **PENDING**
6. **Phase 5:** list any non-2.5 follow-ups the audit surfaced in a new "2.5 follow-ups" section at the bottom of this file. ⏳ **PENDING** (preliminary follow-ups listed above)
