 ...
 *
 * We explicitly exclude `public/` (which has the runtime render
 * data: `beats.json`, `timestamps.json`, `narration.mp3`,
 * `sfx-ambient.mp3`) and `out/` (smoke-test artifacts:
 * `smoke.png`, `last-render.json`) so Vitest never tries to walk
 * them. `node_modules/` and `dist/` are excluded by default but
